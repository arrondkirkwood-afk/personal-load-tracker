...
assert.ok(html.includes('Oilfield Load &amp; Workday Tracker') && html.includes('Add Additional Paid Time'), 'new app identity and paid-time workflow are visible');
...
assert.ok(script.includes("category === 'Deadhead'"), 'Deadhead paid-time records remain readable');
  ...
  assert.ok(script.includes("scrollIntoView({ behavior: 'smooth', block: 'start' })"), 'Add Paid Time action opens and scrolls directly to the form');
...
assert.ok(repairHtml.includes('index.html?v=1.20.1'), 'repair page opens the current version');
...
assert.strictEqual(appVersionMatch[1], '1.20.1', 'app version is updated');
