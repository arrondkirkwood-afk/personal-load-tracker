...
assert.ok(html.includes('Oilfield Load &amp; Workday Tracker') && html.includes('id="paid-time-title">Add Paid Time'), 'new app identity and paid-time workflow are visible');
assert.ok(html.includes('<option>Vacation Time</option>'), 'Paid Time includes Vacation Time');
assert.ok(html.includes('<option>Deadhead</option>') && html.includes('<option>Truck Wash</option>') && html.includes('<option>Breakdown</option>'), 'Paid Time includes Deadhead, Truck Wash, and Breakdown');
assert.ok(script.includes("category === 'Deadhead'"), 'Deadhead paid-time records remain readable');
  assert.ok(html.includes('id="show-paid-time-button"'), 'dashboard exposes a direct Add Paid Time action');
  assert.ok(script.includes("paidTimeControls.panel.hidden = false"), 'Add Paid Time action opens the global paid-time form');
...
assert.ok(html.includes('id="load-add-more"') && html.includes('Deadhead?') && html.includes('Paid wait?') && html.includes('id="has-other-paid-time"'), 'quick questions reveal deadhead, paid-wait, and other paid-time workflows');
...
assert.ok(html.includes(`vendor/exceljs.min.js?v=${appVersionMatch[1]}`), 'versioned Excel workbook library loads before the app');
